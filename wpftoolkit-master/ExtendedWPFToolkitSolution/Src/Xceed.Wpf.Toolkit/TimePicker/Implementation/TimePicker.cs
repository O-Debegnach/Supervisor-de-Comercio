...
namespace Xceed.Wpf.Toolkit
{
    ...
    public class TimePicker : DateTimePickerBase
    {
        ...
        protected override void OnValueChanged(DateTime? oldValue, DateTime? newValue)
        {
            ...
            if (DateTimeUtilities.IsSameDate(Minimum, oldValue)
              != DateTimeUtilities.IsSameDate(Minimum, newValue))
            {
                invalidate = true;
            }

            if (DateTimeUtilities.IsSameDate(Maximum, oldValue)
              != DateTimeUtilities.IsSameDate(Maximum, newValue))
            {
                invalidate = true;
            }
        ...
        protected override void Popup_Opened(object sender, EventArgs e)
        {
            ...
            if (_timeListBox != null)
            {
                ...
                TimeSpan time = (Value != null) ? Value.Value.TimeOfDay : TimePicker.StartTimeDefaultValue;
        ...
        public override void OnApplyTemplate()
        {
            if (TextBox != null)
            {
                TextBox.GotKeyboardFocus -= TextBoxSpinner_GotKeyboardFocus;
            }
            if (Spinner != null)
            {
                Spinner.GotKeyboardFocus -= TextBoxSpinner_GotKeyboardFocus;
            }
        ...
        {
            DateTime date = newDate ?? ContextNow;
            // Set TimePicker TextBox (not Value) to DatetimePicker TextBox.
            if (TextBox != null)
            {
                TextBox.Text = date.ToString(GetFormatString(Format), CultureInfo);
            }
        ...
        private void TimeListBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (e.AddedItems.Count > 0)
            {
                ...
                TimeSpan time = selectedTimeListItem.Time;
                ...
                //else
                //{
                DateTime date = Value ?? ContextNow;
        ...
        public IEnumerable GenerateTimeListItemsSource()
        {
            ...
            if (Value.HasValue)
            {
                ...
                DateTime minDate = Minimum.GetValueOrDefault(System.Globalization.CultureInfo.CurrentCulture.DateTimeFormat.Calendar.MinSupportedDateTime);
                DateTime maxDate = Maximum.GetValueOrDefault(System.Globalization.CultureInfo.CurrentCulture.DateTimeFormat.Calendar.MaxSupportedDateTime);
